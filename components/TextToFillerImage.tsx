...
const TextToFillerImage = (inputString: string, width: number = 90, height: number = 90) => {
    const randomIndex = Math.floor(Math.random() * ColourPalette.length);
    return (
        <View style={{ width: width, height: height, flex: 1, borderRadius: 10, justifyContent: 'center', alignItems: 'center', backgroundColor: ColourPalette[randomIndex] }}>
            <Text style={{ color: '#fff', fontSize: 24, textAlign: 'center' }}>{GetInitialChars(inputString)}</Text>
        </View>
    )
}
